...
class Lego(object):
    def __init__(self, image):
        ...
        self._has_corner_points = False
        self._has_affine_matrix = False
        self._has_affined_image = False
        self._has_affined_corner_points = False

        #the main part of the algorithm
        self._logo_detect(image)

        self._get_corner_points()
        self._get_affined_image_corner_points()
    ...
    def _logo_detect(self,image):
        ...
        if len(new_contours) >= 1:
            ...
            self._cnt = sorted(new_contours, key=cv2.contourArea, reverse=True)[0]

            # compute the rotated bounding box of the contour
            rect = cv2.minAreaRect(self._cnt)
    ...
    def _get_corner_points(self):
        if self._has_potential_logo:
            ...
            if(len(cptaContourPts) is 4):
                ...
                self._corner_points = cPts
                self._has_corner_points = True

    def _get_affined_image_corner_points(self):
        if self._has_corner_points:
            img = self._image
            imgH,imgW, _ = img.shape
            l = imgPreprocessing.LogoAffinePos(self._pureLogo,featureObject=cv2.AKAZE_create(), matcherObject=cv2.BFMatcher(), \
                                                    matchMethod = 'knnMatch')
            cPts, rcvM, flag = l.getRcvAffineInfo(self._cnt,self._corner_points)
            if flag is False:
                return
            else:
                img = cv2.warpPerspective(img, rcvM, (imgW,imgH))
                ...
                cropedImgLogo = img[yMin:yMax,xMin:xMax]
                try:
                    Ang = l.getRotInfo(cropedImgLogo, None)
                except:
                    pass
                if Ang is None:
                    return
                else:
                    cPts, rcvM, rtnFlag = l.getRcvAffineInfo(self._cnt,self._corner_points, -Ang)
                    if (rtnFlag is False):
                        return
            self._affined_image = cv2.warpPerspective(img, rcvM, (imgW,imgH))
            self._affined_corner_points = cv2.perspectiveTransform(cPts.copy().astype('float32'),rcvM)
            self._has_affined_image = True
            self._has_affined_corner_points = True
    ...
    def get_affined_image(self):
        return self._affined_image

    def get_affined_corner_points(self):
        if self._has_affined_corner_points:
            return self._affined_corner_points